...
import numpy as np
from charot.solver.KS_solver import KS
import torch


def contourplot_KS(
    uu, dt=0.5, num_plot_points=1000, plot_frame=False, frameskip=1, filename=""
):
    ...
    ct = ax.contourf(
        x,
        tt[:num_plot_points][:num_plot_points][::frameskip],
        uu[:num_plot_points][::frameskip],
        61,
        cmap=cm.RdBu,
        vmin=-2.5,
        vmax=2.5,
...
def main():
    ...
    action2 = torch.tensor(
        np.random.uniform(size=ks.num_actuators), dtype=torch.float32
    )
