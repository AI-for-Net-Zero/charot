...
def make_ks_env(cfg):
    ...
    buffer_required = cfg.network.architecture == 'buffer_lstm' or \
                      cfg.network.architecture == 'attentionBuffer' or \
                      cfg.network.architecture == 'attentionLSTMBuffer'
    if buffer_required:
        transform_list.append(
...
def make_tqc_agent(cfg, train_env, eval_env):
    ...
    if cfg.network.architecture == 'base':
        actor_net = basic_actor(cfg, action_spec)
    ...
    if cfg.network.architecture == 'base':
        ...
        critic = SelfAttentionMemoryCritic(cfg, action_spec, algorithm="TQC")
    elif cfg.network.architecture == 'attention2':
